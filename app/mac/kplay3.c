...
void audio_callback(void *userdata, Uint8 *stream, int len)
{
    printf("sdl audio cb [len]=[%d]\n", len);
    AVCodecContext *aCodecCtx = (AVCodecContext *) userdata;
    int len1, audio_size;

    // aFrame->nb_samples * aFrame->channels * sizeof(uint16_t);
    ...
    while (len > 0) 
    {
        ...
        {
            // 已经将全部解码后的数据送出去了, 再获取
            audio_size = audio_decode_frame(aCodecCtx, aFrame, &audio_buf);
            printf("debug [asize, audio_buf]=[%d, %p]\n", audio_size, audio_buf);
            if (audio_size >= (1024 * 10))
            {
                printf("error [asize]=[%d] is larger than [%d]\n", audio_size, (1024 * 10));
            }

            if (audio_size <= 0)
            {
                printf("error asize is zero\n");
                // 获取失败, 输出静音
                // audio_buf_size = 1024;
            ...
            else
            {
                ...
                // memcpy(audio_buf, aFrame->data[0], audio_buf_size);
            }
            audio_buf_index = 0;
        }
...
int audio_decode_frame(AVCodecContext *aCodecCtx, AVFrame *aFrame, uint8_t **pcm)
{
    ...
    if (got_output)
    {
        ...
        *pcm = av_malloc(out_size);
        printf("malloc [pcm, out_size]=[%p, %d]\n", *pcm, out_size);
        // 指针数组
        uint8_t **out = pcm;
        printf("swr_convert begin\n");
        ...
        printf("swr_convert end nb_samples from [%d] to [%d]\n", aFrame->nb_samples, len);
        int resample_size = len * 2 * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);
        printf("swr_convert end size from [%d] to [%d]\n", org_size, resample_size);
        return resample_size;
        // return aFrame->nb_samples * aFrame->channels * sizeof(uint16_t);
        // return aFrame->linesize[0];
    }
    if (pkt.data)
    {
        av_free_packet(&pkt);
