...
| 名称     | 说明                                                         |
| -------- | ------------------------------------------------------------ |
| ID3V2    | 位于mp3文件的首部, 包含作者, 作曲, 专辑等信息, 长度不固定, 扩展了ID3V1的信息量 |
| 音频数据 | 1. 音频数据由一系列的数据帧构成<br />2. 每个帧的长度可能固定, 也可能不固定, 由位率决定<br />3. 每个帧都包括帧头, 数据实体两部分<br />4. 帧头记录着mp3的位率, 采样率, 版本等信息<br />5. 如果启用CRC校验, 则帧头后跟随2字节的CRC校验, 之后可能存在32字节的附件信息, 也可能无 |
...
| ---- | ----------- | ------------------------------------ |
| 4    | FF FB 94 00 | 帧头                                 |
| 32   | 00          | 通道信息(第一个数据帧有, 后面的没有) |
| Var  |             | 帧内容                               |

前4个字节参考下面:

<img src="./data/mp3-frame-header.png" style="zoom:100%"/>

举例: 

FF = 1111 1111

FB = 111[同步信息]1 1[版本]01[层]1[是否有CRC校验]

94 = 1001[位率] 01[采样率]0[是否需要调整文件头长度]0[保留字]

00 = 00[声音模式]00[Joint Stereo外加信息] 0[是否合法]0[是否原版]00[降噪压缩后的补偿分类]

同步信息: 11位bit, 都必须是1, 所以第一个字节恒为FF

版本: 2位bit, 11表示MPEG1

层: 2位bit, 01表示Layer3

是否有CRC校验: 1位bit, 1表示无CRC校验

位率: 4位bit, 1001表示128kbps

采样率: 2位bit, 01表示48kHz

是否需要调整文件头长度: 1位bit. 0表示无需调整

保留字: 1位bit

声道模式: 2位bit, 00表示立体声Stereo

Joint Stereo外加信息: 2位bit, 如果不是Joint Stereo就用不到

是否合法: 1位bit, 0表示不合法

是否原版: 1位bit, 0表示非原版

降噪压缩后的补偿分类: 2位bit, 未定义



CBR的音频数据, 每帧的采样数是固定的, 通过  ((采样数/8 * 位率)/采样率 + 填充)  可以获得每帧的长度(bytes), 减去36个bytes, 就是声音内容的长度(bytes)

<img src="./data/mp3-samples-per-frame.png" style="zoom:100%"/>

所以这支mp3的frameSize = (1152 / 8 * 128000) / 48000 + 0 = 384 (bytes)

减去frame-header固定的4 + 32 = 36 (bytes)

真正的音频长度是384 - 36 = 348 (bytes)



<img src="./data/mp3-vbr-frame.png" style="zoom:100%"/>
